...
const Sidebar = ({
    ...
    return (
        <aside className="w-1/5 bg-white border-r border-gray-200 p-6 sticky top-14 h-screen overflow-y-auto hidden lg:block shadow-lg">
            <div className="mb-8">
                    ...
                    <div className="flex flex-col gap-2">
                        {Object.keys(executiveData[selectedYear] || {}).map(group => (
                            <button
                                ...
                                className={`w-full text-left px-4 py-2 rounded-lg font-medium transition-all duration-300 flex items-center gap-2 ${selectedGroup === group ? "bg-emerald-600 text-white shadow-md transform scale-[1.02]" : "bg-gray-100 text-gray-700 hover:bg-emerald-50 hover:text-emerald-600 border border-gray-200"}`}
                            >
                                <Users className="h-4 w-4" />
                                <span>{groupLabels[group] || group}</span>
                                <span className={`ml-auto text-xs px-2 py-1 rounded-full ${selectedGroup === group ? 'bg-white/20' : 'bg-gray-200 text-gray-500'}`}>
